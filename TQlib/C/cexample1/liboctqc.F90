...
! 141222 BOS modified to use new version of liboctq.F90
!
!\begin{verbatim}
...
! These are automatically stored in cnam
!     do i=1,20
!        cnames(i)=' '
!     enddo
...
! tv dimensioned (20,24) in C meaning 24 characters for each element
     do i=1,nel
        j=len_trim(cnam(i))
        do k=1,j
           tv(24*(i-1)+k)=ichar(cnam(i)(k:k))
        enddo
! terminate each name with a zero to please the C program
        tv(24*(i-1)+j+1)=0
     enddo
!-------------------------------------------
  case(4) ! number of phase tuples, stored in ntup
!     call tqgnp(nc,ceq)
!     if(gx%bmperr.ne.0) goto 900
     iv=ntup
!-------------------------------------------
  case(5) ! list name of phase
     cnames(1)=' '
     nc=iv+1
