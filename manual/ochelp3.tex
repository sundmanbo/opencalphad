...
two papers published about OC~\cite{15Sun1,15Sun2}.  General
...
\item \& are used in some parameter identifiers to specify the
  constituent for the parameter, like for mobilites, the mobility
...
easier to to find errors occuring when running macro files.
...
usually no problem unless several equilibria are entered because the
composition set must be deleted in all equilibria.  Composition sets
are created and deleted during normal equilibrium calculations to
detect miscibility gaps.
...
\begin{tabular}{llll}
 BIBLIOGRAPHY   &  ELEMENT        &  OPTIMIZE\_COEFF &  SPECIES\\
 COMMENT        &  EQUILIBRIUM    & PARAMETER        & SYMBOL\\
 CONSTITUTION   &  EXPERIMENT     & PHASE            & TPFUN\_SYMBOL\\
 COPY\_OF\_EQUILIB &  MANY\_EQUILIBRIA & QUIT\\
\end{tabular}
...
HEAD giving the necessary phase status, conditions, expeiments etc.
...
\item Optinonal listing of state variables\\
  Table head: {\bf list HM tc(bcc)}

\item Optional experimental data\\
  Table head: {\bf experiment x(liquid,cr)=0.12:.01, x(bcc,cr)=0.28:.02}
      
\item Optional comment\\
  Table head: {\bf comment experimental data from Kubaschwki 1955}
...
for all columns referrenced in the table head like:\\
Table row: {\bf EQ1 1573 BCC 0.3 0.05}
...
given set of conditions.  If $T$ and $P$ are conditions and all other
conditions are mass balance cnditions $H.T$ is the heat capacity.
...
The only formats implemented at present is SCREEN and TDB.
...
Lists the equilibria calculated during STEP or MAP commands.
...
times more gridpoints) when generating start values for the interative
...
\bibitem[15Sun1]{15Sun1} B Sundman, U R Kattner, M Palumbo and S G
  Fries, Integrating Materials and Manufacturing Innovation, {\bf 4}:1
  (2015), open access
\bibitem[15Sun2]{15Sun2} B Sundman, X-L Liu and H Ohtani,
