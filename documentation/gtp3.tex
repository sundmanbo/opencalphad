...
numerical calculations and there are many numerical libraries
available. It is also intended for use in massive parallell processing.
...
writes a new LaTeX documentation file which require editing but at
least is correct.
...
the system are described by models for the Gibbs energy which can be
...
phase.  With suitable transformations the Gibbs energy can be used to
find the equilibrium of a system for any other set of conditions.  The
model function can also be used to extraplate to non-equilibrium
states.  Some thought has been spent on special physical properties
like ferromagnetism and additional external variables as stress or
mability data that also depend on $T, P$ and the constitution.
...
course been delayed.  The new release date is expected in January 2015.
...
The main feature of version 2 is instead the step and map procedures
which are documented separately and implementing derivatives of state
...
$T, P$ and its constitution.  In addition it can calculte the first
and second derivatives of $G$ with respect to $T, P$ and the
constitution.  These values can be used directly or within a
minimization package developed in parallel, to handle equilibrium
...
\subsubsection{The value of the user defined reference state}\label{sc:calrefstate}
