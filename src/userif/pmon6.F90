...
!3201            format(i3,i5,2x,3(1x,i4),5x,3i4,5x,a)
             enddo
             write(*,3333)
3333         format('MM Number in cat1/2/3 columns is cation index,'/&
                  'Number 1, 2 or 3 in T/0 columns refers to ',&
                  'the cat1/2/3 COLUMN, NOT CATION INDEX!'/&
                  'Asymmetry code is KKK for symmetric, KKT if cat3 is Toop')
!
...
3125            format(/'No such ternary'/)
             ...
             if(index(tersys(asymter)%asymm,'T').gt.0) then
                write(*,*)'MM Ternary is already asymmetric: '
...
! clumsy way to check this is the MQMQA phase
             lokcs=phasetuple(iph)%lokvares
             parres=>ceq%phase_varres(lokcs)
!             if(.not.allocated(parres%mqmqaf)) then
...
!          write(*,*)'Not implemented yet'
!-------------------------------------------------------------------
! list MQMQA_SPECIAL phase ..
!    character (len=16), dimension(mqmqacc) :: mqmqalist=&
!        ['QUADS           ','ASYMMETRIES     ','DEBUG           ',&
!         'EXCESS          ','INTERNAL        ','                ']
       case(22)
! depreciated command
          write(*,*)'MM Depreciated command, use list phase ... mqmqa_var'
!          goto 100
! allow output file
!          lut=optionsset%lut
! if errs not allocated no optimization made
...
4122         format('Fractions of ',i2,' in Quad order, x_ij:',/(12F6.3/))
4123         format('Fractions of ',i2,' in species OC alphabetical order:',/&
                  (12F6.3/))
!
...
4126            format('quad2compvar: ',21(1x,i2))
                write(kou,4127)mqmqa_data%nquad,mqmqa_data%emquad
4127       format('Number of asymmetric varkappa_ij variables, n*(n-1)/2: ',i3/&
                'em2quad: ',21(1x,i2))
! just a blank line
                write(kou,*)
...
!
             write(kou,444)'MM Values of y_i/k: ',&
                 (ceq%phase_varres(lokcs)%mqmqaf%y_ik(i1),i1=1,mqmqa_data%ncat)
444          format(/a,(10f7.4))
             write(kou,4444)
4444         format('MM Indexing of x_ij (i=1,n; j=i,n) :')
! an empty line before extended information
! NOTE THIS IS PART OF REDUNDANT COMMAND LIST MQ <phase> ASYMMETRIES
!             call gparcdx('Details on varkappa?',cline,last,1,ch1,'N',&
!                  '?Varkappa')
             mqmqavar=>ceq%phase_varres(lokcs)
! Now this incluse also y_i/k and x_ij equations and values
!             call varkappadefs(mqmqavar)
             call list_mqmqa_variables(mqmqavar)
!
!...........................................................
