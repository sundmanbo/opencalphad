...
!\addtotable subroutine config_entropy_mqmqa2
!\begin{verbatim}
! subroutine config_entropy_mqmqa(moded,ncon,phvar,phrec,tval)
 subroutine config_entropy_mqmqa2(phvar,moded,lokph,tval)
!
! SUPERSEEDED by config:entropy_mqmqa2: smaller arrays
!
...
! Do we need to save the original value for model parameters??
   write(*,*)'3X in config_entropy_mqmqa',lokph
   ncon=phlista(lokph)%tnooffr
!   phrec=phlista(lokph)
   ddebug=.TRUE.
!   ddebug=.FALSE.
...
!      conname=splista(phrec%constitlist(mqmqa_data%contyp(10,s1)))%symbol
      conname=splista(phlista(lokph)%constitlist(mqmqa_data%contyp(10,s1)))%symbol
      connames(s1)=conname
      if(ddebug) write(*,3)s1,(mqmqa_data%contyp(s2,s1),s2=5,14),&
           (mqmqa_data%constoi(s2,s1),s2=1,4),phvar%yfr(s1),trim(conname)
3     format('3X mq:',i2,1x,4i2,1x,i3,1x,4i2,1x,i2,4i3,4F5.1,F5.2,1x,a)
   enddo
   if(ddebug) then
      do s1=1,ncon
         write(*,4)s1,(mqmqa_data%pp(s2,s1),s2=1,4),trim(connames(s1))
4        format('3X pp1:',i2,4(F8.5),2x,a)
      enddo
   endif
!   write(*,'(a,20i3)')'3X pinq: ',(mqmqa_data%pinq(s1),s1=1,mqmqa_data%npair)
!   write(*,6)phvar%yfr
...
! calculates configurational entropy/R for the FactSage MQMQA model
 end subroutine config_entropy_mqmqa2
...
! allocate arrays
   ddebug=.FALSE.
!   ddebug=.TRUE.
   if(ddebug) write(*,*)'3X in calc_mqmqa to calculate G nonconfig'
...
!   write(*,*)'3X assigning mqf pointer'
   mqf=>phres%mqmqaf
!   write(*,*)'3X assigning mqf pointer OK'
!-------------------
...
!            write(*,'(a,2i3,2(1pe12.4))')'3X SNN endmember',mqmqj,kend,&
!                 pyq,vals(1)
...
!         write(*,'(a,2i3,2(1pe12.4))')'3X FNN endmember',mqmqj,kend,&
!                 pyq,vals(1)
         if(ipy.eq.1) then
...
! but add reference state parameters to all SNN and reciprocal constituents
   ipy=1
   if(ddebug) write(*,*)'3X adding reference to SNN endmembers'
...
205      format(a,2i3,F8.5,2x,3(1pe12.4))
         if(ddebug) &
          write(*,210)'3X FNN G:     ',mqmqj,mqmqj,pyq,aff,pyq,phres%gval(1,1)
...
! %contyp(1..4,mqmqj) is index of FNN reference energy
         if(ddebug) write(*,'(a,i3,1x,4i3,4F8.5)')'3X pp2: ',mqmqj,&
...
211         format('3X SNN dG/dy:',3i3,1(1pe12.4))
            do itp=1,3
               phres%dgval(itp,mqmqj,ipy)=phres%dgval(itp,mqmqj,ipy)+&
                    aff*refg(zp,itp)
            enddo
!           write(*,212)zp,mqmqj,ipy,phres%dgval(1,mqmqj,ipy),aff,aff*refg(zp,1)
212         format('3X SNN dG/dy reference added:',3i3,3(1pe12.4))
! Initially ignore 2nd derivatives, d2G/dy2=1/y set by entropy calculation
            if(ddebug) write(*,213)s1,zp,mqmqj,ipy,pyq,aff,phres%gval(1,ipy)
213         format('3X SNN G ref:',4i3,2F10.5,1pe12.4)
            do itp=1,6
               phres%gval(itp,ipy)=phres%gval(itp,ipy)+pyq*aff*refg(zp,itp)
            enddo
...
214         format('3X SNN G ref added:',3i3,4(1pe12.4))
...
!                 (phres%dgval(1,s2,1),s2=1,gz%nofc)
   ...
   enddo qloop
   if(ddebug) write(*,*)'3X finished loop for endmembers'
...
! NOTE dx12, dx21 and sihma are arrays as any constituent can be involved
         dx12(gz%iq(1))=(jdeg+1)*dxrk
         dx21(gz%iq(2))=(jdeg+1)*dxrk
! derivatives with respect to additional fractions in x12, x21 and sigma
! not taken care of ...
