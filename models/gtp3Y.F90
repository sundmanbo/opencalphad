...
!\begin{verbatim} %-
 subroutine generate_ionliq_grid_old(mode,iph,ngg,nrel,xarr,garr,ny,&
      yarr,gmax,ceq)
...
111 format('3Y EM: ',10(i4,i3)/11(i4,i3))
!   gx%bmperr=4399; goto 1000
!---------------------------------------
! We have now generated endm(1..nsl,j)
!120 continue
         ...
         endmem3: do kend=1,nend
            ...
            endmem4: do lend=1,nend
               ...
               if(mode.eq.0) then
                  ...
                  if(gx%bmperr.ne.0) goto 1000
                  if(lutbug.gt.0) then
! debug output for OLD version of ionic liquid
                     write(lutbug,710)ngg,nrel,ncon,garr(ngg),&
                          (xarr(jj,ngg),jj=1,nrel),(yfra(jj),jj=1,ncon)
710                  format('OLD: ',i5,i3,1pe10.2,10(0pF6.3))
                  endif
                  if(garr(ngg).gt.gmax) gmax=garr(ngg)
...
!   call get_phase_data(iph,1,nsl,nkl,knr,ydum,sites,qq,ceq)
   ...
   return
 end subroutine generate_ionliq_grid_old
...
!\begin{verbatim}
 subroutine gridmin_check_old(nystph,kp,nrel,xarr,garr,xknown,ngrid,pph,&
      cmu,yphl,iphx,ceq)
...
!   integer idum(1000)
   save addph
!
!   write(*,*)'3Y Entering gridmin_check_old',addph
   gstable=zero
   nystph=0
   mode=0
...
1000 continue
   return
 end subroutine gridmin_check_old
